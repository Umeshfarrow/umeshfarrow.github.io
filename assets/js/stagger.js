...
function animateEl(el, index, variant, duration, stagger, threshold, initialDelay = 0) {
  ...
  const [x, y, scale, rotate] = getTransform(variant, index);
...
function getTransform(variant, index = 0) {
  ...
  const dist = 44;
  switch (variant) {
    case 'up':     return ['0',          `${dist}px`,  '1',    '0deg'];
    case 'down':   return ['0',          `${-dist}px`, '1',    '0deg'];
    case 'left':   return [`${-dist}px`, '0',          '1',    `${parity * 2}deg`];
    case 'right':  return [`${dist}px`,  '0',          '1',    `${parity * -2}deg`];
    case 'scale':  return ['0',          '0',          '0.85', '0deg'];
    case 'scale-up': return ['0',        `${dist}px`,  '0.8',  '0deg'];
    case 'tilt':   return ['0',          '24px',       '0.92', `${parity * 3}deg`];
    case 'fade':   return ['0',          '0',          '1',    '0deg'];
    case 'fade-left': return [`${-dist}px`, '0',       '0.92', `${parity * 2}deg`];
    default:       return ['0',          '32px',       '1',    '0deg'];
  }
}
